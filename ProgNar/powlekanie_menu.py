...
class CoatingMenu:
    ...
    def __init__(self, parent, update_callback):
        ...
        self.coating_options = list(self.coating_data.keys())
        self.length_options = []
    ...
    def toggle_coating_options(self):
        ...
        if self.coating_frame.winfo_ismapped():
            ...
            self.coating_combo["values"] = self.coating_options if self.coating_options else ["Brak danych"]
            self.length_combo["values"] = self.length_options if self.length_options else ["Brak danych"]
            if self.coating_options:
                self.coating_var.set(self.coating_options[0])
    ...
    def get_coating_price(self, diameter):
        ...
        try:
            ...
            if not (coating and length and diameter > 0):
                return 0.0
    ...
    def get_coating_params(self):
        ...
        coating = self.coating_var.get()
        length = self.length_var.get()
        if coating and length:
            return {"Powłoka": coating, "Długość całkowita": length}
        return {}